...
class Game extends React.Component {
    ...
    gameTick = () => {
        var body = [];
        this.state.snakeList.forEach((i) => body.push(i));
        tcount = (tcount + 1) % 40;

        var newInc = this.state.increment;
        var newx = (this.state.snakeList[0][0] + newInc[0]), newy = (this.state.snakeList[0][1] + newInc[1]);
        if (this.state.snakeList.filter((i) => { return i[0] === newx % 19 && i[1] === newy % 19; }).length || newx < 0 || newx > 18 || newy < 0 || newy > 18) {
    ...
    componentDidMount() {
        ...
        keyboard.addEventListener('keydown', e => {
            ...
            if (this.state.snakeList.filter((i) => { return i[0] === newx % 19 && i[1] === newy % 19; }).length || newx < 0 || newx > 18 || newx < 0 || newx > 18) {
                this.setState({ gameOver: true });
            } else {
